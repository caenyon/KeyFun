...
def handle_keyboard_all(event):
    ...
    # print "-------", str(virtual_key), event.KeyID, event.IsExtended(), event.IsTransition(), event.IsInjected(), \
    #     event.ScanCode
    #return True

    # Workaround: ALT-TAB does not work since win 8 if the keys are generated by the program. so redirect the
    # physically pressed TAB key when the alt key is pressed.
